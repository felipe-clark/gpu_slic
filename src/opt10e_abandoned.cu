...
, bool h_debug) {
    ...
    int x = (blockIdx.x * blockDim.x + threadIdx.x) * pix_at_a_time_x;
    int y = (blockIdx.y * blockDim.y + threadIdx.y) * pix_at_a_time; //See Opt6
    int sx = (x % (32 * pix_at_a_time_x)) / pix_at_a_time_x;
    ...
    int i_center = (blockIdx.x * blockDim.x * pix_at_a_time_x) / spx_size;
    ...
    //if (cc==0)
    //{
    for (int idx=0; idx<pix_at_a_time*pix_at_a_time_x; ++idx)
    {
	int yidx = idx % pix_at_a_time;
	if (y+yidx>=pix_height) break;
	int xidx = idx / pix_at_a_time;
	if (x+xidx>=pix_width) break;
        int pix_index = (y + yidx) * pix_width + x + xidx;
        int i = d_own_data[pix_index].i;
        int j = d_own_data[pix_index].j;
	...
	// so should be coalesced (and aligned, if pix_data is padded)
        acc[0][ny][nx][sy][sx] = d_pix_data[pix_index].l + (idx?acc[0][ny][nx][sy][sx]:0);
        acc[1][ny][nx][sy][sx] = d_pix_data[pix_index].a + (idx?acc[1][ny][nx][sy][sx]:0);
        acc[2][ny][nx][sy][sx] = d_pix_data[pix_index].b + (idx?acc[2][ny][nx][sy][sx]:0);
        acc[3][ny][nx][sy][sx] = 1 + (idx?acc[3][ny][nx][sy][sx]:0);
    }
    //}
