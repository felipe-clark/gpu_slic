...
, bool h_debug) {
    ...
    __shared__ int acc[4][3][3][8][33]; //LAB+count, 3x3 neighbors, 8x32 values (33 for bank conflict avoidance)
    const int arraySize = 4 * 3 * 3;
    const int dimensions = 8 * 33; // Adjusted for mem bank conflict avoidance
